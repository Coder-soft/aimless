...
public class AimlessConfigScreen {
    ...
    public static Screen build(Screen parent) {
        ...
        ConfigBuilder builder = ConfigBuilder.create()
                ...
                .setTitle(Component.literal("Aimless Config"));

        ConfigEntryBuilder entryBuilder = builder.entryBuilder();

        ConfigCategory aim = builder.getOrCreateCategory(Component.literal("Aim Point"));

        int bodyPct = (int) Math.round(config.getBodyHeight() * 100);
        aim.addEntry(entryBuilder.startIntSlider(
                Component.literal("Body Height"),
                bodyPct, 0, 100)
                .setDefaultValue(80)
                .setTextGetter(v -> {
                    double pct = v;
                    ...
                    return Component.literal(String.format("%s (%.0f%%)", label, pct));
                })
                .setTooltip(Component.literal("Aim height on the target's body.\n0% = feet, 100% = top of head"))
        ...
        aim.addEntry(entryBuilder.startIntSlider(
                Component.literal("Horizontal Offset (X)"),
                offX, -400, 400)
                .setDefaultValue(0)
                .setTextGetter(v -> Component.literal(String.format("%+.2f blocks", v / 100.0)))
                .setTooltip(Component.literal("Left/right offset from body center.\n-4.00 to +4.00 blocks"))
        ...
        aim.addEntry(entryBuilder.startIntSlider(
                Component.literal("Forward Offset (Z)"),
                offZ, -400, 400)
                .setDefaultValue(0)
                .setTextGetter(v -> Component.literal(String.format("%+.2f blocks", v / 100.0)))
                .setTooltip(Component.literal("Forward/backward offset from body center.\n-4.00 to +4.00 blocks"))
                .setSaveConsumer(v -> config.setOffsetZ(v / 100.0))
                .build()
        );

        aim.addEntry(entryBuilder.startIntSlider(
                Component.literal("Reaction Ticks"),
                config.getReactionTicks(), 1, 100)
                .setDefaultValue(6)
                .setTextGetter(v -> Component.literal(v + " ticks (" + (v * 50) + "ms)"))
                .setTooltip(Component.literal("Delay between aim corrections.\n1 tick = 50ms, 20 ticks = 1 second"))
